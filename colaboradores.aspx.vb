...
#Region "Parametrización de Jefes"
    ...
    Public Sub obtCatalogoJefes()
        ...
        Try
            ...
            odbComando.CommandText = "dnc_muestra_colaboradores_sp"
            ...
            odbComando.Parameters.AddWithValue("@usuario_creacion", hdUsuario.Value)
            ...
            grdColaboradores.DataSource = dsCatalogo.Tables(0).DefaultView
            grdColaboradores.DataBind()

            If grdColaboradores.Rows.Count = 0 Then
                divAvance.Visible = False
                lblMensaje.Text = "No hay Colaboradores para configurar."
            Else
                divAvance.Visible = True
                lblMensaje.Text = ""
            End If
            ...
            For i = 0 To grdColaboradores.Rows.Count - 1
                Dim iIdColaborador As String
                Dim ddlJefe As DropDownList
                Dim strTipo As String = ""
                Dim btnEditar As LinkButton = grdColaboradores.Rows(i).Controls(4).Controls(0)

                iIdColaborador = DirectCast(grdColaboradores.Rows(i).Cells(0).FindControl("lblId"), Label).Text

                If btnEditar.Text <> "Editar" Then
                    ddlJefe = grdColaboradores.Rows(i).Cells(1).FindControl("ddlJefe")
                    Call obtddlColaboradores(ddlJefe)
                    'obtiene provedor

                    For iContador As Integer = 0 To dsCatalogo.Tables(0).Rows.Count - 1
                        If dsCatalogo.Tables(0).Rows(iContador)(0).ToString = iIdColaborador Then
                            ddlJefe.SelectedValue = dsCatalogo.Tables(0).Rows(iContador)(3).ToString
                        End If
                    Next
                End If
            ...
            For iFil As Integer = 0 To grdColaboradores.Rows.Count - 1
                If iFil Mod 2 = 0 Then
                    grdColaboradores.Rows(iFil).BackColor = Color.FromName("#F2F2F2")
                End If
            Next
            'carga años
    ...
    'Obtiene los colaboradores activos
    Public Sub obtddlColaboradores(ByVal ddl As DropDownList)
        Dim sConnString As String = ConfigurationManager.ConnectionStrings("sqlConn").ConnectionString
        Dim odbConexion As OleDbConnection = New OleDbConnection(sConnString)
        Dim dsCatalogo As New DataSet
        ...
        strQuery = " SELECT clave,	[NOMBRE] + ' '+[APPAT] +' '+[APMAT] as nombre FROM SGIDO_INFOGIRO_GIRO_VT WHERE estatus='ACTIVO'  ORDER BY 2"
    ...
    Private Sub grdColaboradores_RowCancelingEdit(sender As Object, e As GridViewCancelEditEventArgs) Handles grdColaboradores.RowCancelingEdit
        grdColaboradores.ShowFooter = True
        grdColaboradores.EditIndex = -1
        Call obtCatalogoJefes()
    End Sub

    'TOOLTIPS
    Private Sub grdColaboradores_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles grdColaboradores.RowDataBound
    ...
    Private Sub grdColaboradores_RowEditing(sender As Object, e As GridViewEditEventArgs) Handles grdColaboradores.RowEditing
        grdColaboradores.ShowFooter = False
        grdColaboradores.EditIndex = e.NewEditIndex
        Call obtCatalogoJefes()
    End Sub
    'actualiza la descripcion
    Private Sub grdColaboradores_RowUpdating(sender As Object, e As GridViewUpdateEventArgs) Handles grdColaboradores.RowUpdating
        grdColaboradores.ShowFooter = True
        Dim sConnString As String = ConfigurationManager.ConnectionStrings("sqlConn").ConnectionString
        Dim odbConexion As OleDbConnection = New OleDbConnection(sConnString)
        Dim strQuery As String
        ...
        Try
            strId = DirectCast(grdColaboradores.Rows(e.RowIndex).FindControl("lblId"), Label).Text


            strClave = (DirectCast(grdColaboradores.Rows(e.RowIndex).FindControl("ddlJefe"), DropDownList).Text)
            strNombre = (DirectCast(grdColaboradores.Rows(e.RowIndex).FindControl("ddlJefe"), DropDownList).SelectedItem.Text)
            odbConexion.Open()
            ...
            strQuery = "UPDATE [dbo].[DNC_JEFE_PROVISIONAL_TB]" & _
                        "SET [fk_id_jefe] = '" & strClave.ToUpper & "'" & _
                        "   ,[nombre_jefe] = '" & strNombre & "'" & _
    ...
    Protected Sub grdColaboradores_PageIndexChanging(sender As Object, e As GridViewPageEventArgs) Handles grdColaboradores.PageIndexChanging
        Try
            grdColaboradores.ShowFooter = True
            grdColaboradores.EditIndex = -1
